...
class Annotation:
	...
	def mouse_event_handler(self, event):
		...
		if self.annotation_mode == "polygon":
			# if 0 <= self.previous_index < len(self.polys_to_render):
			...
			if event.type == gui.MouseEvent.Type.BUTTON_DOWN:
				...
				def is_click_on_box_boundary(mouse_x, mouse_y, x1, y1, x2, y2):
					...
					print(x1, y1, x2, y2)
					if (image_x >= x1 - 3 and image_x <= x1 + 3 and image_y >= y1 - 3 and image_y <= y2 + 3) or \
					(image_x >= x2 - 3 and image_x <= x2 + 3 and image_y >= y1 - 3 and image_y <= y2 + 3) or \
					(image_x >= x1 - 3 and image_x <= x2 + 3 and image_y >= y1 - 3 and image_y <= y1 + 3) or \
					(image_x >= x1 - 3 and image_x <= x2 + 3 and image_y >= y2 - 3 and image_y <= y2 + 3):
						return True
					else:
						return False
				...
				def is_click_inside_box(mouse_x, mouse_y, x1, y1, x2, y2):
					image_x, image_y = transform_mouse_to_image(widget, mouse_x, mouse_y)
				...
				if not self.drag_edge and not self.drag_corner and not self.drag_box and self.previous_index != -1:
					...
					if is_click_inside_box(mouse_x, mouse_y, corners[0], corners[1], corners[2], corners[3]):
						self.drag_box = True
						self.curr_x = mouse_x
						self.curr_y = mouse_y
						print('box drag started')
						return gui.Widget.EventCallbackResult.CONSUMED
			...
			if event.type == gui.MouseEvent.Type.DRAG:
				...
				box = self.boxes_to_render[self.previous_index]
				corners = box[CORNERS]
				print("prev_index while dragging:", self.previous_index)
				...
				if self.drag_corner:
					...
					corners[0] += image_delta_x
					corners[1] += image_delta_y
					corners[2] += image_delta_x
					corners[3] += image_delta_y
	...
	def update_props(self):
		if self.annotation_mode == "polygon":
			boxes = [self.annotation_class, self.delete_annotation_button, self.confidence_set]
			enabled = False
	...
	def exit_annotation_mode(self):
		if (self.save_check == 0 and self.temp_polys != self.old_polys) or \
			(self.save_check == 0 and self.temp_pred_polys != self.old_pred_polys):
	...
	def confirm_exit(self):
		if hasattr(mask_lct.Window, '__init__'):  # Checking if __init__ method exists
		...
		if self.image_window:
			self.image_window.close()  # Assuming self.image_window is properly instantiated
		if self.cw:
			self.cw.close()  # Assuming self.cw is properly instantiated and callable
	...
	def update_mask(self):
		...
		print("Annotations saved to output_annotations.json")
