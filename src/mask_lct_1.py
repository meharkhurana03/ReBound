...
# Integrate the CDAL bash script to this code so that important images are selected and we can annotate them.
...
import platform
# local import
import mask_annotation_editing as edit
...
def parse_options():
    ...
    # Use the argparse options
    input_path = args.feature  # or any other option you want to use
...
class Window:
	...
	def __init__(self, lct_dir, frame_num=0):
		...
		self.path_string = os.path.join(self.lct_path ,"mask", str(self.frame_num), "polys.json")
		...
		try:
			for i in range(0, self.num_frames):
				...
				polys = json.load(open(os.path.join(self.lct_path ,"mask", str(i), "polys.json")))
		...
		try:
			for i in range(0, self.pred_frames):
				...
				polys = json.load(open(os.path.join(self.lct_path,"pred_mask", str(i), "polys.json")))
				# for box in boxes['boxes']:
				for poly in polys['polys']:
	...
	def on_annotation_start(self):
		...
		annotation_object = edit.Annotation(self.frame_extrinsic, self.polys, self.pred_polys,
...
class kCenterGreedy():
    ...
    def update_distances(self, cluster_centers, only_new=True, reset_dist=False):
        ...
        if cluster_centers:
            ...
            dist = pairwise_distances(self.features, x, metric=kl)
    ...
    def select_batch(self, already_selected, N):
        ...
        return new_batch, selected_indices  # Return selected indices
...
class ImageSelectorApp:
    ...
    def prev_image(self):
        ...
        if self.current_index < 0:
            self.current_index = len(self.image_paths) - 1  # Wrap around to the last image
        self.load_image()
        self.display_image()
...
def main():
    ...
    select, selected_indices = np.sort(kc.select_batch(already_selected, target_pick))

    for s in selected_indices:
        selected_names.append(unlabeled_names[s])

    print('Selected Paths:', selected_names)

    # Start GUI
    root = tk.Tk()
    app = ImageSelectorApp(root, selected_names)
    root.mainloop()
...
if __name__ == "__main__":
    ...
    if not testing.is_lct_directory(args, mask=True):
